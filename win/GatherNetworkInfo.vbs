'. Windows 7 provides a very sophisticated WSH script (over 1K loc) which dumps
'. out configuration data to file in text format.  The script provides many
'. examples, and much reuseable code for querying system settings via the WMI
'. and various commandline utilities.
'.
'. Here's the script in it's entirity:
